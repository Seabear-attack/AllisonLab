...
% Load experimental pulse
wavelength_cutoff_low_nm = 750;
wavelength_cutoff_high_nm = 830;
c_nm_per_fs = 299792458 * 10^-6;

I_exp_file = readmatrix("C:\Users\wahlm\Documents\School\Research\Allison\" + ...
    "Tunable Pump\EDFA\6-14-23\1.2A all, ~25cm PM1550.csv");

time = I_exp_file(2:end, 1);
truncated_indexes = find((I_exp_file(1,:)>wavelength_cutoff_low_nm) & (I_exp_file(1,:)<wavelength_cutoff_high_nm));
I_exp_truncated = I_exp_file(:, truncated_indexes);
wavelength_nm = I_exp_truncated(1, :);
time_fs = time*1e-15;
D = time_fs;
dt = time_fs(2)-time_fs(1);
...
for ni=1:1   % number of random realizations
    if alg==1
        [Obj, error, Irec] = ePIE_fun_FROG(I_exp_freq, D, 200, Fsupp, F', time_pulse_reconstruction, 1e-3);
    else
        [Obj, error, Irec] = ePIE_fun_FROG_sp(I_LPF, D, 200, Fsupp, F', time_fs, 1e-3, abs( fft(ref)/1 ));
    end  
...
subplot(3, 2, 1); imagesc(time_fs*1e15, fftshift(F)*1e-15, sqrt(abs(I)))
xlabel('Time [fsec]','FontSize',16); ylabel('Freq.[THz]','FontSize',16);
title('Simulated trace')

subplot(3, 2, 2); imagesc(time_fs*1e15, fftshift(F)*1e-15, sqrt(abs(Inoisy)))
xlabel('Time [fsec]','FontSize',16); ylabel('Freq.[THz]','FontSize',16);
title('Simulated noisy trace')

subplot(3,2,3)
plot(time_fs*1e15, abs(ObjB))
xlabel('Time [fsec]','FontSize',16); ylabel('Amplitude [a.u.]','FontSize',16);
hold all
plot(time_fs*1e15, abs(ref))
xlabel('Time [fsec]','FontSize',16); ylabel('Amplitude [a.u.]','FontSize',16);
subplot(3,2,4)
plot(time_fs*1e15, unwrap(   angle(ObjB)   )/pi)
xlabel('Time [fsec]','FontSize',16); ylabel('Phase [pi]','FontSize',16);
hold all
plot(time_fs*1e15, unwrap(   angle(ref)   )/pi)
xlabel('Time [fsec]','FontSize',16); ylabel('Phase [pi]','FontSize',16);
...
subplot(3,2,5)
imagesc(time_fs*1e15, fftshift(F)*1e-15, sqrt(abs(Icut)))
xlabel('Time [fsec]','FontSize',16); ylabel('Freq.[THz]','FontSize',16);
title('Used trace')

subplot(3, 2, 6); imagesc(time_fs*1e15, fftshift(F)*1e-15, sqrt(abs(Irec)))
xlabel('Time [fsec]','FontSize',16); ylabel('Freq.[THz]','FontSize',16);
