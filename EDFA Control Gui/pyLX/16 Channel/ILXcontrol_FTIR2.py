# -*- coding: utf-8 -*-
"""
Created on Thu Jun  8 12:23:01 2023
...
class ILXcontrol:           # To create an instance of this class: ilx = ILXcontrol()
    ...
    def init_amps(self):
        ...
        # EDFA2
        self.EDFA2_channels_fwd = [5, 6]   # Hard-coded amplifier-diode selections. (change as needed)
        self.EDFA2_channels_bwd = [7, 8]
        ...
        # EDFA3
        self.EDFA3_channels_fwd = [9, 10]   # Hard-coded amplifier-diode selections. (change as needed)
        self.EDFA3_channels_bwd = [11, 12]
        self.EDFA3_channels = self.EDFA3_channels_fwd + self.EDFA3_channels_bwd
        print('The pre-set EDFA3 channels are fwd: '+str(self.EDFA3_channels_fwd)+' and bwd: '+str(self.EDFA3_channels_bwd)+'.')

        self.EDFA3_currents = 0 #initialize EDFA3 currents
        print('The pre-set EDFA3 currents are '+str(self.EDFA3_currents)+' mA.')
        
        # EDFA4
        self.EDFA4_channels_fwd = [13, 14]   # Hard-coded amplifier-diode selections. (change as needed)
        self.EDFA4_channels_bwd = [15, 16]
        self.EDFA4_channels = self.EDFA4_channels_fwd + self.EDFA4_channels_bwd
        print('The pre-set EDFA4 channels are fwd: '+str(self.EDFA4_channels_fwd)+' and bwd: '+str(self.EDFA4_channels_bwd)+'.')

        self.EDFA4_currents = 0 #initialize EDFA4 currents
        print('The pre-set EDFA4 currents are '+str(self.EDFA4_currents)+' mA.')
    ...
    def LDI_set(self, ch, ImA_set):
        ...
        above_limit = False
        if ch in self.EDFA1_channels+self.EDFA2_channels+self.EDFA3_channels+self.EDFA4_channels:
    ...
    def EDFA3_on(self):
        i = 0
        for ch in self.EDFA3_channels:
            self.LAS_onoff(ch, 1)           # Now turn on LAS and TEC
            time.sleep(self.sleeptime)                   # Wait a moment
            self.LDI_set(ch, self.EDFA3_currents)         # And set currents to specified values.
            time.sleep(self.sleeptime)                   # Wait a moment
            i += 1
        self.beep()
    ...
    def EDFA4_on(self):
        i = 0
        for ch in self.EDFA4_channels:
            self.LAS_onoff(ch, 1)           # Now turn on LAS and TEC
            time.sleep(self.sleeptime)                   # Wait a moment
            self.LDI_set(ch, self.EDFA4_currents)         # And set currents to specified values.
            time.sleep(self.sleeptime)                   # Wait a moment
            i += 1
        self.beep()
