...
class ILXcontrol:           # To create an instance of this class: ilx = ILXcontrol()
    ...
    def init_serial(self):
        ...
        self.serialpath = "/dev/ttyUSB0"  # Specify path to serial-to-usb port where device is plugged.
        self.serialpath = "/dev/ttyS0" # Test direct serial port connection
    ...
    def init_amps(self):
        ...
        # FTIR aeroDiode
        self.FTIR_channels = [10]
        print('The pre-set FTIR channels is: '+str(self.FTIR_channels)+ '.')

        self.FTIR_currents = [0]
        print('The pre-set preAmp currents are '+str(self.FTIR_currents)+' mA.')
    ...
    def init_Ilim(self):
        if self.testmode == False:
            ...
            self.Ilim_set_aero = 250.0 # (Note: this is much lower than the 3S diodes!))
            self.s.write('CHANNEL 10;LAS:LIM:I {:.1f};\n'.format(
                self.Ilim_set_aero).encode('ascii'))
            time.sleep(self.sleeptime)
            self.s.write(b'CHANNEL 10;LAS:LIM:I?;\n')  # Choose a representative
            time.sleep(self.sleeptime)
            self.Ilim_aero = self.s.readline()
            time.sleep(self.sleeptime)
            Ilim_aero_str = self.Ilim_aero.decode().strip()
            print('aerodiode channel LAS current limits set to '+Ilim_aero_str+' mA')
