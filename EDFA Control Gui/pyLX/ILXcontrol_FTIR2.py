...
class ILXcontrol:  # To create an instance of this class: ilx = ILXcontrol()
    ...
    def __init__(self):
        self.ImA_last = None
        self.TEC_stat_float = None
        self.LAS_stat_float = None
        self.Ilim_3S = None
        self.Ilim_set_3S = None
        self.EDFA2_currents = None
        self.EDFA2_channels = None
        self.EDFA2_channels_bwd = None
        self.EDFA2_channels_fwd = None
        self.EDFA1_currents = None
        self.EDFA1_channels = None
        self.EDFA1_channels_bwd = None
        self.EDFA1_channels_fwd = None
        self.s = None
    ...
    def init_serial(self):
        self.testmode = True  # True = testing; False = real serial connection
        ...
        if not self.testmode:
            self.s = serial.Serial(self.serialpath,
                                   timeout=1)  # Open the serial connection to specified path = here/is/port
            ...
            if self.s.is_open:  # Output ‘true’ verifies that the port is indeed open
    ...
    def init_amps(self):
        ...
        # EDFA1
        self.EDFA1_channels_fwd = [5, 6]  # Hard-coded amplifier-diode selections. (change as needed)
        self.EDFA1_channels_bwd = [13, 14]
        self.EDFA1_channels = self.EDFA1_channels_fwd + self.EDFA1_channels_bwd
        print('The pre-set EDFA1 channels are fwd: ' + str(self.EDFA1_channels_fwd) + ' and bwd: ' + str(
            self.EDFA1_channels_bwd) + '.')

        self.EDFA1_currents = 0  # initialize EDFA1 currents
        print('The pre-set EDFA1 currents are ' + str(self.EDFA1_currents) + ' mA.')

        # EDFA2
        self.EDFA2_channels_fwd = [1, 2]  # Hard-coded amplifier-diode selections. (change as needed)
        self.EDFA2_channels_bwd = [7, 8]
        self.EDFA2_channels = self.EDFA2_channels_fwd + self.EDFA2_channels_bwd
        print('The pre-set EDFA2 channels are fwd: ' + str(self.EDFA2_channels_fwd) + ' and bwd: ' + str(
            self.EDFA2_channels_bwd) + '.')
    ...
    def open_serial(self):
        if not self.testmode:
            if self.s.is_open:
                ...
                self.s.open()
                if self.s.is_open:  # Should be true
    ...
    def close_serial(self):
        ...
        if not self.testmode:
            if not self.s.is_open:
                ...
                self.s.close()
                if not self.s.is_open:  # Should be false
                    print("Disconnected ILX serial port.")
                if self.s.is_open:  # Should be false
    ...
    def LAS_onoff(self, ch, onoff):
        ...
        self.LDI_setzero(ch)
        # time.sleep(self.sleeptime) if onoff == 1: self.s.write(('CHANNEL ' + str(ch) + '; TEC:OUT ' + str(onoff) + 
        # ';\n').encode()) #turn on TEC before turning on the diodes print('Ch.'+str(ch))
        time.sleep(self.sleeptime)
        self.s.write(
            ('CHANNEL ' + str(ch) + '; LAS:OUT ' + str(onoff) + ';\n').encode())  # turn on LAS -> turning on diodes.
        time.sleep(self.sleeptime)
