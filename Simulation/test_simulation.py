...
pulse_freq_THz = np.linspace
np.interp

pulseWL = 1565  # pulse central wavelength (nm)
rep_rate = 61   # MHz
# a_t = np.array([])
EPP = (4e-9) / 8  # Energy per pulse (J) + fudge factor
...
pulse.set_time_window_ps(pulse_time_ps)
pulse.set_frequency_window_THz(c_nm_per_ps / pulse_wavelength_nm)
...
zW = np.transpose(AW)[:, (F > 0)]
zW_dB = dB(np.transpose(AW)[:, (F > 0)])
zT = np.transpose(AT)
zT_dB = dB(np.transpose(AT))
